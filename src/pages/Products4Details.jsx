...
function Products4Details() {
  const { id } = useParams();
  const { addToCart, cart, increaseQuantity, decreaseQuantity } = useCart();

  const product = Cbfabrics.find((item) => item.id === id);
  ...
  if (!product) return (
    <>
      ...
      <div className="flex flex-col items-center justify-center h-screen">
        ...
        <Link to="/Products4" className="mt-4 text-red-800 font-semibold underline">
          Go back
        </Link>
      </div>
    </>
  ...
  return (
    <>
      ...
      <div className="contain pt-16 px-4">
        {/* Back button */}
        <div className="py-4">
          <Link to="/Products4" className="text-red-800 font-semibold">
            ← Back to Products
          </Link>
        </div>
...
export default Products4Details;