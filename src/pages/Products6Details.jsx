...
function Products6Details() {
  const { id } = useParams();
  const { addToCart, cart, increaseQuantity, decreaseQuantity } = useCart();

  const product = Vintages.find((item) => item.id === id);
  ...
  if (!product) return (
    <>
      ...
      <div className="flex flex-col items-center justify-center h-screen">
        ...
        <Link to="/Products" className="mt-4 text-red-800 font-semibold underline">
          Go back
        </Link>
      </div>
    </>
  ...
  return (
    <>
      ...
      <div className="contain pt-16 px-4">
        {/* Back button */}
        <div className="py-4">
          <Link to="/Products6" className="text-red-800 font-semibold">
            ← Back to Products
          </Link>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-8 items-center justify-center py-8">
          ...
          <div className="details flex flex-col gap-4 max-w-md">
            ...
            {/* Cart controls */}
            {cartItem ? (
              <div className="flex items-center justify gap-4 mt-4">
                <button
...
export default Products6Details;