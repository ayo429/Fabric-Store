...
import { Dmfabrics } from '../Data/Dmfabrics';
import Navbar from '../shared components/Navbar';
import { useCart } from '../Context/Cartcontext';

function Products5Details() {
  const { id } = useParams();
  const { addToCart, cart, increaseQuantity, decreaseQuantity } = useCart();

  const product = Dmfabrics.find((item) => item.id === id);
  ...
  if (!product) return (
    <>
      ...
      <div className="flex flex-col items-center justify-center h-screen">
        ...
        <Link to="/Products5" className="mt-4 text-red-800 font-semibold underline">
          Go back
        </Link>
      </div>
    </>
  ...
  return (
    <>
      ...
      <div className="contain pt-16 px-4">
        {/* Back button */}
        <div className="py-4">
          <Link to="/Products5" className="text-red-800 font-semibold">
            ← Back to Products
          </Link>
        </div>
          ...
          <div className="details flex flex-col gap-4 max-w-md">
            ...
            {cartItem ? (
              ...
              <button
                ...
                className="mt-4 bg-black text-white px-8 py-3 rounded-lg font-semibold w-full md:w-auto">
                Add to Cart
              </button>
            )}
            ...
            {/* Checkout link */}
            <Link to="/Checkout">
              <button className="bg-red-800 text-white px-8 py-3 rounded-lg font-semibold w-full md:w-auto">
                Go to Checkout
              </button>
            </Link>
          </div>
...
export default Products5Details;